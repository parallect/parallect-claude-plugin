...
You have access to Parallect, a multi-provider deep research platform. It
queries multiple frontier AI providers simultaneously and synthesizes their
...
Parallect's tools come from its hosted MCP server, configured by this plugin
(`.mcp.json` → `https://parallect.ai/api/mcp/mcp`). Tools appear as
`mcp__parallect__research`, `mcp__parallect__get_results`, and so on.

On first use, Claude Code will prompt you to authenticate to Parallect:

- **OAuth (recommended):** sign in through the browser when prompted. No key to
  manage.
- **API key:** alternatively, set a `par_live_*` key (create one at
  https://parallect.ai/settings/keys). See the README for client config.

If the Parallect tools are not available, the connection isn't authenticated
yet — tell the user to run `/mcp` (or reconnect) and sign in.

## Gotchas

Read these first. They prevent the most common mistakes:

- **Research is async.** Calling `research` does NOT return results. It returns
  a `jobId`. You must poll `research_status`, then call `get_results` only
...
- **Balance check before research is mandatory.** If the user has no balance
  and no payment method, the research call will fail. Check first.
- **The `synthesis` field is markdown.** Present it as-is with citations. Don't
  strip the formatting.
- **`research` always creates a new thread.** Do NOT pass a `threadId` to the
...
| `search_claims` | User wants to find specific claims across research | No research has been run |
| `get_claim_evidence` | User wants the evidence chain behind a specific claim | You don't have a claim id |
...
this right builds trust; getting it wrong causes bill shock.
...
The `research` tool always creates a new thread. Do NOT pass a `threadId`. For
...
**Cap the maximum interval at 120 seconds.** Never poll more frequently than
every 30 seconds.

On each poll, report progress naturally ("1 of 3 providers finished, 2 still
running...", "All providers done, synthesis is running now...").
...
1. Call `get_results` with `jobId`.
   - `includeProviderReports: false` unless the user wants raw per-provider data.
   - `includeClaimsJson: false` unless the user wants structured claims.

2. Present findings in this order:
   a. **Brief summary** (3-5 sentences) of the key findings in your own words.
      Lead with the most important insight.
   b. **Full synthesis** — share the `synthesis` markdown as-is. It contains
      inline citations and cross-references.
   c. **Cost report** — "This research cost $X.XX across N providers (took Y
      minutes)."
   d. **Balance check** — call `balance` and report remaining. If below $5,
      flag it.

3. If the job `status` is `"failed"`, report what happened and suggest retrying
   with a different tier or fewer providers.
...
When the user picks one, use `follow_up` with the parent `jobId` and
`topicIndex` (0-based) or `customQuery`. Mention the estimated cost before
submitting, then repeat Steps 3-5. Follow-ups run in the same thread, so
providers have full context.

## Cost Awareness Rules

You are spending the user's money at machine speed. Be responsible:

- **NEVER auto-submit research in a new session** without first establishing
  budget expectations. Non-negotiable.
- **Track cumulative session spend** and report it after multiple queries.
- **Suggest cheaper tiers proactively** when the question is simple.
- **Confirm expensive operations** (L/XL tiers, chains of follow-ups).
- **Report cost after EVERY research job.** Never skip the cost report.
- **Alert at low balance** (below $5 mention it; below $2 warn the next research
  might fail).
...
`references/api-errors.md` for full error codes, response fields, and recovery
steps.

## Tips for Quality Research Queries

- **Be specific:** "What are the latest Phase 3 clinical trial results for
  GLP-1 receptor agonists in treating NASH?" beats "GLP-1 drugs".
- **Include constraints:** time period, geography, sector, comparison criteria.
- **If the user is vague, ask ONE clarifying question** before spending money.
- **Don't over-scope.** Match the query scope to the budget tier.
...
- Remember `threadId` and `jobId` from the current research context.
- If the user says "the research" or "those results", use the most recent
  jobId. Don't ask them to repeat it.
- To resume older research, call `list_threads`, then `get_thread` for context.
- Track cumulative spend across the session.
- `research` creates a new thread every time; `follow_up` continues an existing
  one. Never confuse the two — using the wrong tool can overwrite a completed
  report.
